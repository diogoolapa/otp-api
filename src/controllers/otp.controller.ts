...
import { Metrics } from '../services/otp/metrics.service';
...
export async function handleOtpRequest(req: OtpRequest, reply: FastifyReply) {
  ...
  if (!parsed.success) {
    return reply.status(400).send({ error: 'Invalid body', issues: z.flattenError(parsed.error) });
  }
  const { identifier, channel } = parsed.data;

  const limit = Number(env.RATE_LIMIT_PER_MINUTE ?? '3');
  const ok = await checkRateLimit(`${identifier}:${req.ip ?? 'unknown'}`, limit, 60);
  if (!ok) {
...
export async function handleOtpVerify(req: OtpVerify, reply: FastifyReply) {
  ...
  try {
    ...
    otpVerifyFail.inc();
    if (result.status === 'expired') return reply.status(410).send({ success: false, error: 'Code expired' });
    if (result.status === 'blocked') return reply.status(429).send({ success: false, error: 'Too many attempts' });
